...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  if(!created.ok)return reply({error:'Your sighting could not be saved. Please try again.'},502);
  ...
  try{
   for(let i=0;i<photos.length;i++){const file=photos[i];const ext=file.type==='image/jpeg'?'jpg':file.type==='image/png'?'png':file.type==='image/webp'?'webp':file.type==='image/heic'?'heic':'heif';const path=`${sighting.id}/${String(i+1).padStart(2,'0')}-${randomUUID()}.${ext}`;const bytes=Buffer.from(await file.arrayBuffer());
    const up=await supa(`storage/v1/object/sighting-photos/${path}`,{method:'POST',headers:{'Content-Type':file.type,'x-upsert':'false'},body:bytes});if(!up.ok)throw new Error('photo upload failed');uploaded.push(path);
    const meta=await supa('rest/v1/sighting_photos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sighting_id:sighting.id,storage_path:path,original_filename:file.name.slice(0,500),mime_type:file.type,size_bytes:file.size})});if(!meta.ok)throw new Error('photo metadata failed');
   }
