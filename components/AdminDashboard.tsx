...
export default function AdminDashboard(){
  ...
  async function saveStatus(nextStatus:string){
    ...
    setSelected({...selected,status:nextStatus});setBusy(true);setNotice('');
    try{
      await editorApi('updateStatus',{id:selected.id,status:nextStatus});
      await loadSightings();
      setNotice('Status updated — '+statusLabel(nextStatus)+'.');
    }
    catch(e){
      setSelected(current=>current&&current.id===selected.id?{...current,status:previousStatus}:current);
