...
export function validateSubmission(data){
 ...
 if(kind==='sighting'){
  ...
  if(!text('location')||text('location').length>500)errors.push('Enter the sighting location.');
  const n=Number(text('count'));if(!Number.isInteger(n)||n<1||n>10000)errors.push('Enter a whole number of flamingos between 1 and 10,000.');
 ...
 if(photos.length>3||photos.some(f=>f.size>20*1024*1024)||photos.reduce((s,f)=>s+f.size,0)>MAX_BYTES||photos.some(f=>!types.includes(f.type)))errors.push('Use up to 3 JPG, PNG, WebP, HEIC, or HEIF photos, up to 20 MB each.');
 return {errors,photos};
}
export function imageSignatureMatches(bytes,type){if(type==='image/jpeg')return bytes[0]===255&&bytes[1]===216&&bytes[2]===255;if(type==='image/png')return bytes.slice(0,8).join(',')==='137,80,78,71,13,10,26,10';if(type==='image/webp')return new TextDecoder().decode(bytes.slice(0,4))==='RIFF'&&new TextDecoder().decode(bytes.slice(8,12))==='WEBP';if(type==='image/heic'||type==='image/heif')return new TextDecoder().decode(bytes.slice(4,12)).includes('ftyp');return false;}